    ...
    <main className="min-h-screen bg-background pt-24 pb-20">
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="mb-10 text-center md:text-left">
          {post.categories && post.categories.length > 0 && (
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-6">
              {post.categories.map((cat) => (
                <span
                  key={cat.id}
                  className="px-3 py-1 rounded-full bg-cream dark:bg-brand-orange-deep/30 text-brand-orange-deep dark:text-brand-orange-light text-sm font-semibold"
                >
                  {cat.name}
                </span>
              ))}
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-3 text-sm text-muted border-y border-border py-5">
            <div className="flex items-center gap-2">
              <div className="relative w-9 h-9 rounded-full overflow-hidden bg-gradient-to-br from-brand-orange-deep to-brand-orange flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          {/* Main Content */}
          <div className="flex-1 max-w-3xl min-w-0 prose prose-lg dark:prose-invert prose-headings:font-bold prose-a:text-brand-orange-deep dark:prose-a:text-brand-orange-light hover:prose-a:text-brand-orange prose-pre:overflow-x-auto prose-img:rounded-2xl">
          ...
          <aside className="w-full lg:w-72 flex-shrink-0 space-y-8">
            {/* Author Profile */}
            <div className="bg-card rounded-2xl p-6 border border-border shadow-sm lg:sticky lg:top-24">
              <h3 className="text-sm font-bold uppercase tracking-wider text-muted mb-4">
                Written By
              </h3>
              <div className="flex items-center gap-4 mb-4">
                <div className="relative w-16 h-16 rounded-full overflow-hidden bg-gradient-to-br from-brand-orange-deep to-brand-orange flex items-center justify-center text-white text-xl font-bold shadow-lg flex-shrink-0">
                  {isValidImageSrc(post.author?.image) ? (
                    <Image src={post.author!.image!} alt={post.author?.name || "Author"} fill className="object-cover" />
                  ) : (
                    (post.author?.name || "T").charAt(0)
              ...
              {(authorSocial.website || authorSocial.twitter || authorSocial.linkedin) && (
                <div className="flex items-center gap-3 mb-6">
                  {authorSocial.website && (
                    <a href={authorSocial.website} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-background text-muted hover:text-brand-orange-deep dark:hover:text-brand-orange-light transition-colors">
                      <Globe size={16} />
                    </a>
                  )}
                  {authorSocial.twitter && (
                    <a href={authorSocial.twitter} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-background text-muted hover:text-brand-orange-deep dark:hover:text-brand-orange-light transition-colors">
                      <Twitter size={16} />
                    </a>
                  )}
                  {authorSocial.linkedin && (
                    <a href={authorSocial.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-background text-muted hover:text-brand-orange-deep dark:hover:text-brand-orange-light transition-colors">
                      <Linkedin size={16} />
                    </a>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {post.tags.map((tag) => (
                      <span
                        key={tag.id}
                        className="inline-flex items-center gap-1 px-3 py-1 rounded-md bg-background text-muted text-xs font-medium"
                      >
                        <Tag size={12} />
                        {tag.name}
                      </span>
      ...
      {relatedPosts.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 pt-16 border-t border-border">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {relatedPosts.map((related) => (
              <Link
                ...
                className="group flex flex-col bg-card rounded-2xl border border-border overflow-hidden hover:shadow-xl hover:shadow-brand-orange/10 transition-all duration-300"
              >
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <h3 className="font-bold text-foreground mb-2 line-clamp-2 group-hover:text-brand-orange-deep dark:group-hover:text-brand-orange-light transition-colors">
                    {related.title}
                  </h3>
